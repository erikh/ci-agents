...
// PostTasksCancelID swagger:route POST /tasks/cancel/{id} postTasksCancelId
// Cancel by Task ID
...
func PostTasksCancelID(h *handlers.H, ctx *gin.Context, processingHandler handlers.HandlerFunc) *errors.Error {
	...
	if err := PostTasksCancelIDValidateURLParams(h, ctx); err != nil {
		return errors.New(err)
	}

	if processingHandler == nil {
		return errors.New("'/tasks/cancel/{id}': no processor defined")
	}
	...
	return PostTasksCancelIDResponse(h, ctx, resp, code, err)
}
